...
class PointCloudDistanceNode(Node):
    ...
    def maskCallback(self, img_msg, mask_msg):
        for detection in mask_msg.detections:
            if (len(detection.mask.data) > 0):
                self.get_logger().info("receving masks..")
                points = np.array([[int(ele.y), int(ele.x), 0] for ele in detection.mask.data])
                print(points)
                self.publishCloud(points, img_msg, self.mask_pub)
